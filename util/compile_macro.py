...
def generate_cache():
    ...
    def goodheader(x):
        ...
        classbypath={"root6":"T", "R3BRoot":"", "FairRoot":"Fair",
                     "root":"T", "fairroot":"Fair", "vmc":"T"}
        ok=False
        for k,v in classbypath.items():
            ok |= dirname.find(k)!=-1 and basename.startswith(v)
        # header files containing autocompile are probably autogenerated
        # by us, ignore them. 
        ok &= dirname.find("autocompile")==-1
        ok &= len(basename)>3
        if not ok:
    ...
    include_search_pathes=filter(lambda a:len(a)>0, include_search_pathes)
    ...
    library_search_pathes=filter(lambda a:len(a)>0, library_search_pathes)
...
def try_link(base):
    ...
    r=run(["g++", "-g", "-o", base.elf(), "--std=c++17", base.obj()]
