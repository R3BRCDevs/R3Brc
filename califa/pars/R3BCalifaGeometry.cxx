...
const ROOT::Math::XYZVector& R3BCalifaGeometry::GetCrystalCenterPos(int id)
{
    static std::array<std::optional<ROOT::Math::XYZVector>, 10000> cache;
    Double_t local[3] = { 0, 0, 0 };
    Double_t master[3];
    const static ROOT::Math::XYZVector invalid(NAN, NAN, NAN);
    
    const char* nameVolume;
    auto& res=cache.at(id);
    if (res)
      return res.value();

    if (id > fNumChannels / 2 && id <= fNumChannels)
      res = GetCrystalCenterPos( id - fNumChannels / 2);
    else if (id >= 1 && id <= fNumChannels / 2)
    {
        ...
        if (gGeoManager->CheckPath(nameVolume))
	  {
	    ...
	    res = {master[0], master[1], master[2]};
	  }
        else
	  {
